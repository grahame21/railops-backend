...
LOCO_XLSX = os.path.join("static", "downloads", "loco_database.xlsx")
...
def loco_database_download():
    if not os.path.exists(LOCO_XLSX):
        return cors(make_response(jsonify({
            "ok": False,
            "error": "loco_database.xlsx not generated yet"
        }), 404))
    resp = make_response(send_file(
        LOCO_XLSX,
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        as_attachment=True,
        download_name="loco_database.xlsx",
    ))
    return cors(resp)
...
def root():
    return cors(make_response(jsonify({
        "ok": True,
        "hint": "Use /trains.json or /downloads/loco_database.xlsx"
    })))
